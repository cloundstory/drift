...
function drawSnow(ctx,list){
  for(const t of list){
    ...
    if(a<=.02) continue;
    const rad=0.85+0.85*t.w;
    ctx.fillStyle='rgba(255,254,250,'+a.toFixed(3)+')';
...
function drawFog(ctx,list){
  ...
  for(const b of list){
    ...
    /* ⚠️ getImageData คืนสี *ก่อนผสมกับพื้นหลังหน้าเว็บ*
       พิกเซลที่ alpha ต่ำจึงคืนสีหมึกเข้ม ไม่ใช่สีที่ตาเห็นจริง
       เอามาทาตรง ๆ = ได้ก้อนดำ ต้องผสมกับสีกระดาษเองก่อน */
    let tone='rgba(226,224,216,';
    try{
      const px=sc.getImageData(Math.round(p.x*dpr),Math.round(p.y*dpr),1,1).data;
      const al=px[3]/255;
      const r=Math.round(px[0]*al+PAPER[0]*(1-al));
      const gg=Math.round(px[1]*al+PAPER[1]*(1-al));
      const bb=Math.round(px[2]*al+PAPER[2]*(1-al));
      tone='rgba('+r+','+gg+','+bb+',';
    }catch(e){}
...
function drawIce(ctx,list,cracks){
  ...
  B.forEach((arr)=>{
    for(let i=0;i<arr.length;i+=5){
      const gl=arr[i+4];
      if(gl<0.25) continue;
      ctx.fillStyle='rgba(255,255,252,'+(gl*0.8).toFixed(3)+')';
      ctx.beginPath(); ctx.arc(arr[i],arr[i+1],1.15,0,6.2832); ctx.fill();
    }
  });
  ...
  for(const c of cracks){
    ...
    ctx.strokeStyle='rgba(46,54,72,'+a.toFixed(3)+')';
    ctx.beginPath();
    ctx.moveTo(p.x-s,p.y-s*0.6); ctx.lineTo(p.x+s,p.y+s*0.6);
